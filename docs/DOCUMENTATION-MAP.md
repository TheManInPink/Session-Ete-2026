...
|  11 | AI-SERVICE-FASTAPI             | (existant) | ADR-004, ADR-015          | ✅ implémenté — 7 endpoints `/api/v1/ai` + `/health` (`app/`), scorer heuristique ; doc décrit 5 endpoints/`src/` → drift §6 #13. PROMPT 4.2 : dataset-generator + features 14→20 + `names.json` (AUC 0.98) → drift §6 #14 |
...
|  14 | Dataset IA enrichi (PROMPT 4.2) ↔ features désalignées : 80 % des erreurs visent les noms, mais `*_is_common` ne connaissait que ~20 noms (vs 350+ au catalogue) → **AUC ≈ 0.58**                                                                        |    ✅    | `services/ai-service/app/services/{reference,features,scorer}.py` · `ai-models/dataset-generator/` · `data/mali/names.json` | ✅ Résolu 2026-06-14 : `reference.py` charge `names.json` (∪ embarqué) ; `features.py` 14→20 ; scorer passe la date brute ; re-train → **AUC 0.98 / F1 0.98** |
...
2026-06-14  ai-models/dataset-generator (PROMPT 4.2) + alignement IA features 14→20 + data/mali/names.json  (AUC 0.58→0.98)
```
