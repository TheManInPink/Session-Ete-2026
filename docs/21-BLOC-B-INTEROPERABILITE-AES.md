...
>   réponse, signature de l'émetteur — chaîne Merkle compatible audit-service (cf. ADR-014).
...
2. **Défense en profondeur** : mTLS **ET** JWS Ed25519. mTLS prouve « c'est bien la gateway BFA qui
   parle ». JWS prouve « le payload n'a pas été altéré et a été signé par le keystore BFA ».
   Compromis l'un ne suffit pas à forger un appel valide.
