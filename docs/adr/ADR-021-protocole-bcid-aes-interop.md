...
2. **Authentification double-couche** :
   - **mTLS** : chaque gateway présente un cert client X.509 émis par la **CA AES partagée** (3 root
     CAs cross-signed Mali / BFA / NER). Le hash du cert peer est cross-référencé dans la table
     `aes_partner_keys` côté serveur.
   - **JWS Ed25519** sur le payload applicatif. Algorithme `EdDSA`, clés Ed25519 (32 octets, RFC
     8032). Verification via JWK enregistré dans `aes_partner_keys`.
...
6. **Audit cross-border 10 ans** : table `aes_verification_logs` avec chaîne Merkle compatible
   audit-service (cf. ADR-014). Chaque verification (demandée OU répondue) est tracée pour preuve
...
- **Privacy garantie par construction** : impossible de reconstruire une base citoyens à partir des
  ...
  applicative.
- **Audit cryptographique** : chaque verification est signée + chaînée Merkle. Un partenaire
  mal-intentionné qui ferait des requêtes abusives laisse une trace inaltérable.
...
- **Audit ANSSI-compatible** : protocole entièrement documenté (OpenAPI 3.1) + logs
  cryptographiques + procédure d'onboarding partners. Un auditeur peut tracer chaque requête.
...
2. **Clés Ed25519 self-hosted** : générées dans Vault Transit côté chaque pays. Jamais exportées
   hors du datacenter d'origine.
...
Si **disponibilité descend sous 99 %**, ou si **un Merkle chain break** est détecté, déclencher une
revue ADR.
