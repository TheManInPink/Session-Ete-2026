...
| ------ | ------------------------------ | --------------------------------------------------------------------------------------------------------- | ------------------------- | ------------------------------------------------------------------------------------------ |
| ACT-01 | **Citoyen malien**             | Toute personne possédant ou demandant un NINA. Peut être au Mali ou dans la diaspora.                     | Web, Mobile, USSD, Borne  | Email/Téléphone + 2e facteur (TOTP ou passkey préférés ; OTP SMS = fallback, cf. EF-A-011) |
...
| ID        | Exigence                                                                                                                                                                                                                                                                                                                                                                                   | Priorité | Critère d'acceptation                                                                                                                                                           |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| EF-A-011  | Le système doit authentifier les **citoyens** via email ou numéro de téléphone + un **2e facteur fort**. ⚠️ L'OTP SMS seul est faible (vulnérable au SIM-swap et à l'interception SS7) : il est **fallback uniquement**. Le 2e facteur préféré est **TOTP** (RFC 6238) ou une **passkey WebAuthn/FIDO2** (résistante au phishing).                                                         | Must     | TOTP/passkey proposé par défaut ; OTP SMS dégradé (réseau faible) avec validité 5 min, 3 tentatives max ; bannière avertissant l'utilisateur de la fragilité du SMS             |
...
| EF-A-016b | Le système doit appliquer un **rate-limiting GLOBAL et un anti-bruteforce** sur TOUTES les routes d'authentification (login, OTP, vérification 2FA, refresh), pas seulement sur la vérification AES (EF-B-004). Protection par IP, par identité et par dispositif (sliding window + backoff exponentiel), avec CAPTCHA progressif et journalisation des tentatives anormales vers l'audit. | Must     | 10 tentatives/min/IP et 5 échecs/identité → 429 `Retry-After` ; CAPTCHA après 3 échecs ; pic de tentatives → alerte SIGAC/audit. Conçu — implémentation ⏳ Phase 2 si non livré |
| EF-A-016c | Le système doit assurer la **rotation et la révocation des clés de signature JWT/Ed25519** exposées via un endpoint **JWKS** (`/.well-known/jwks.json`), avec période de chevauchement (`kid` multiples) pour ne pas invalider les tokens en vol. Rotation RS256 (signature QR/JWT, Vault Transit) et Ed25519 (scellement audit / échanges AES).                                           | Must     | Rotation ≤ 90 jours (cf. ENF-016), JWKS publie ≥ 2 `kid` pendant le chevauchement, clé révoquée rejetée immédiatement. Conçu — automatisation ⏳ Phase 2 si non livré           |
...
| EF-A-018 | Le système doit former une **chaîne de hash (hash-chain) SHA-256** (PAS un arbre de Merkle ; cf. ADR-007) à partir des entrées d'audit, scellée Ed25519 in-process (@noble/ed25519, doc 09). Garantie d'inviolabilité forte **uniquement si la racine est ancrée chez un tiers** (OCLEI / Vérificateur Général). | Must     | Hash de l'entrée N dépend du hash de l'entrée N-1. Endpoint `/audit/verify` confirme l'intégrité. Ancrage tiers ⏳ Phase 2 si non livré |
...
| EF-A-023 | Le système doit inclure un **QR code JWT RS256** (signé via Vault Transit) contenant : NINA, **identifiant non réversible du template biométrique protégé** (réf. au gabarit ISO 24745 d'EF-F-001 — **jamais** un hash brut de l'empreinte, ni le template lui-même), timestamp d'émission, signature CTDEC. | Must     | QR code scannable, JWT vérifiable avec la clé publique du CTDEC (JWKS, cf. EF-A-016c), expiration configurable, aucune donnée biométrique exploitable dans le QR |
...
#### Portail Citoyen (apps/citizen — port 4001)
...
#### Dashboard Administrateur (apps/admin — port 4002)
...
> **⚠️ Cohérence de périmètre (MoSCoW).** Les exigences EF-F-001 → EF-F-003 décrivent la **cible de
> conception** et sont notées « Must » au sens de la conception (le système ne doit jamais stocker
> de biométrie brute). Pour le **périmètre universitaire effectivement livré**, le Bloc F est classé
> **« Won't have this time »** (cf. §9 et ADR biométrie) : la protection de template ISO 24745
> (fuzzy extractor / cancelable) et le matching 1:1 sont **conçus, Phase 2**, non implémentés. La
> colonne « Priorité » ci-dessous se lit donc « Must **si** le bloc est implémenté ». Cette double
> lecture lève l'incohérence Must vs Won't.

| ID       | Exigence                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Priorité | Critère d'acceptation                                                                                                                                                                                                                |
| -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| EF-F-001 | Le système doit capturer les **empreintes digitales** et n'en stocker qu'une **représentation protégée, révocable et non réversible** conforme à **ISO/IEC 24745** : **biométrie révocable (cancelable biometrics)** ou **fuzzy extractor** (helper data + secret dérivé). ⚠️ Argon2id n'est **PAS** un schéma de template biométrique : un hash cryptographique classique casse le matching tolérant au bruit inhérent à la biométrie ; il reste réservé aux mots de passe (ENF-015). | Must     | Aucune empreinte brute stockée ; template protégé révocable (rotation de la transformation possible sans réenrôlement du doigt) ; non-réversibilité et non-chaînabilité (unlinkability) vérifiées. Conçu — implémentation ⏳ Phase 2 |
...
| ID      | Exigence                                    | Métrique      | Cible                                                                                                                                                                                                                                                                                                                                |
| ------- | ------------------------------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| ENF-013 | Chiffrement des données au repos            | Algorithme    | AES-256-GCM. ⚠️ PostgreSQL (Community) **n'a pas de TDE natif** → chiffrement **applicatif au niveau colonne via Vault Transit** (champs sensibles : PII, biométrie protégée) **+ LUKS** au niveau du volume disque. « TDE » à proscrire dans les docs.                                                                              |
...
| ENF-019 | Audit trail immuable                        | Vérification  | Hash-chain SHA-256 (ADR-007, **pas** un arbre de Merkle) vérifié quotidiennement par job automatique ; scellement Ed25519 in-process ; ancrage racine chez un tiers (OCLEI) ⏳ Phase 2                                                                                                                                               |
| ENF-031 | mTLS inter-services (PKI interne)           | Protocole     | **mTLS strict** entre TOUS les microservices (ADR-034) : chaque service présente un certificat client signé par la PKI interne ; rotation automatique des certificats ; trafic non authentifié rejeté. Conçu — déploiement ⏳ Phase 2 si non livré                                                                                   |
| ENF-032 | En-têtes de sécurité HTTP (apps web)        | Configuration | **CSP** stricte (pas de `unsafe-inline`/`unsafe-eval`), **HSTS** (`max-age` ≥ 1 an, `includeSubDomains`, `preload`), **CORS** allowlist explicite (pas de `*`), plus `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`. Audit via securityheaders.io / observatory.                                                  |
| ENF-033 | Rotation + révocation des clés de signature | Mécanisme     | JWKS (`/.well-known/jwks.json`) publie les clés publiques RS256 (QR/JWT) et Ed25519 (audit/AES) ; rotation ≤ 90 jours avec chevauchement de `kid` ; révocation immédiate d'une clé compromise (cf. EF-A-016c).                                                                                                                       |
| ENF-034 | Rate-limiting global / anti-bruteforce      | Couverture    | Limitation de débit appliquée à **tout le périmètre exposé** (API Gateway + auth + AES), pas seulement aux routes AES : protection par IP/identité/dispositif, backoff exponentiel, 429 `Retry-After` (cf. EF-A-016b).                                                                                                               |
| ENF-035 | Anti-corrélation des lanceurs d'alerte      | Garantie      | Aucun lien stockable entre un signalement (Bloc D) et l'identité/métadonnées du dénonciateur : token de suivi anonyme non corrélable, suppression des métadonnées réseau (IP/UA/timing), chiffrement asymétrique côté client, journaux d'audit ne reliant jamais signalement ↔ session. Conçu — durcissement ⏳ Phase 2 si non livré |
...
| **O3** — Portail citoyen           | EF-A-035 → EF-A-049                                            | ENF-007, ENF-024, ENF-026, ENF-032                             | citizen, admin, mobile                                      |
| **O4** — Interopérabilité AES      | EF-B-001 → EF-B-005                                            | ENF-006, ENF-031, ENF-033, ENF-034                             | interop-service                                             |
| **O5** — Anti-corruption           | EF-D-001 → EF-D-005                                            | ENF-035 (anti-corrélation lanceurs d'alerte), ENF-017, ENF-018 | anticorruption-service                                      |
| **O6** — Gouvernance traçable      | EF-C-005 → EF-C-011                                            | ENF-019, ENF-014 (transit signé/horodaté)                      | governance-service                                          |
...
| **O8** — Sécurité                  | EF-A-011 → EF-A-021, EF-A-016b, EF-A-016c, EF-F-001 → EF-F-003 | ENF-013 → ENF-019, ENF-031 → ENF-035                           | auth-service, audit-service, api-gateway, biometric-service |
| **O9** — Conformité                | EF-A-017 → EF-A-021 (traçabilité/rétention), EF-B-005          | ENF-009 → ENF-012, ENF-024, ENF-027 → ENF-030                  | Transversal                                                 |
...
| **Merkle tree**                    | Structure de données arborescente où chaque nœud contient le hash de ses enfants. ⚠️ **N'est PAS** utilisé pour l'audit NINA-AES : l'audit (ADR-007) repose sur une **hash-chain SHA-256** linéaire, pas un arbre de Merkle. |
| **Hash-chain**                     | Chaîne de hachages linéaire où l'entrée N inclut le hash de l'entrée N-1 — mécanisme d'inviolabilité du journal d'audit (ADR-007), distinct de l'arbre de Merkle                                                             |
| **TOTP**                           | Time-based One-Time Password (RFC 6238) — code à usage unique généré localement par une app authenticator, sans dépendre du réseau SMS                                                                                       |
| **Passkey / WebAuthn / FIDO2**     | Authentification par clé cryptographique liée au dispositif, résistante au phishing — alternative forte à l'OTP SMS                                                                                                          |
| **SIM-swap / SS7**                 | Attaques compromettant l'OTP SMS : détournement de la carte SIM (SIM-swap) ou exploitation du protocole de signalisation SS7 pour intercepter les SMS                                                                        |
| **JWKS**                           | JSON Web Key Set — ensemble de clés publiques (`/.well-known/jwks.json`) permettant la vérification et la rotation des signatures JWT/Ed25519 sans invalider les tokens en vol                                               |
| **CSP / HSTS**                     | Content-Security-Policy et HTTP Strict-Transport-Security — en-têtes HTTP de durcissement (anti-XSS/injection, forçage HTTPS)                                                                                                |
| **Fuzzy extractor**                | Schéma de protection biométrique (helper data + secret reproductible) tolérant le bruit du capteur — alternative ISO 24745 au stockage d'un template brut                                                                    |
| **Cancelable biometrics**          | Biométrie révocable : transformation paramétrée et non réversible du gabarit, permettant de « révoquer » et régénérer un template compromis sans réenrôler la personne (ISO/IEC 24745)                                       |
| **ISO/IEC 24745**                  | Norme de protection des informations biométriques (irréversibilité, révocabilité, non-chaînabilité des templates)                                                                                                            |
...
| **OTP**                            | One-Time Password — code à usage unique. ⚠️ Variante SMS = **fallback faible** (SIM-swap/SS7) ; préférer TOTP ou passkey pour l'authentification citoyen                                                                     |
