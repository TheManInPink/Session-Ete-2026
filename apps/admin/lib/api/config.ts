/**
 * @file        config.ts
 * @description Configuration de la couche données (mode + URLs gateway) pour
 *              la console agent (port 4002).
 *
 *              Isomorphe (serveur RSC + navigateur) : ne lit que `process.env`.
 *              Les variables `NEXT_PUBLIC_*` sont inlinées au build → lisibles
 *              côté client. Pattern miroir d'apps/citizen/lib/api/config.ts.
 *
 ...
 *
 * Refuse de démarrer en production si le mode est `mock` : la console agent
 * afficherait sinon de fausses demandes de correction « valides » — décisions
 * administratives prises sur des données synthétiques. Appelé au boot par
 * `instrumentation.ts`.
 *
...
/**
 * URL **publique** du gateway. Contrairement à citizen, la console agent n'a
 * PAS de transport anonyme (tous les appels sont authentifiés via le BFF) :
 * cette URL ne sert qu'au kill-switch de configuration ci-dessus.
 */
