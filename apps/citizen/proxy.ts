 ...
 *              moteur de rendu via override d'en-tête de requête. `wasm-unsafe-eval`
 *              reste autorisé pour le chiffrement libsodium (PC-06).
 *
 ...
 * et s'appuie sur le nonce + `strict-dynamic`. `wasm-unsafe-eval` : libsodium WASM
 * (PC-06). En dev, `unsafe-eval` est requis par le HMR/React Refresh ; jamais en prod.
 */
function buildCsp(nonce: string): string {
  const scriptSrc = [
    ...
    `'nonce-${nonce}'`,
    "'strict-dynamic'",
    "'wasm-unsafe-eval'", // chiffrement libsodium WASM (PC-06)
    ...(IS_DEV ? ["'unsafe-eval'"] : []),
  ].join(' ');
