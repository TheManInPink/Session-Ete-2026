...
| Catalogue noms IA (`ai-models/dataset-generator/config/names.yml`)  | `python -m dataset_generator.export_reference` → régénère `data/mali/names.json` (lu par `ai-service/.../reference.py`) + re-train `train_xgboost.py`. Noms 100% fictifs → pas concerné par `validate:data` |
| Features IA (`ai-service/app/services/features.py` `FEATURE_NAMES`) | Re-train `ai-models/scripts/train_xgboost.py` (le `.pkl` fige l'ordre des features) + MAJ `docs/11-AI-SERVICE-FASTAPI.md` §7                                                                                |
