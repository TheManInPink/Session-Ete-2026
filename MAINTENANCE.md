...
| Variables IA (`ai-models/training/features.py`)              | Ré-entraîner (`python -m training.train_xgboost`) → nouveau `metadata.json` ; `feature_names` du bundle change ⇒ vérifier l'inférence `ai-service` (le `FeatureBuilder` sérialisé est la source unique) |
| Taxonomie d'erreurs (`error_type` du générateur)             | Aligner `dataset_generator` ↔ `training.data.CANONICAL_LABEL_ORDER`/`_LABEL_SYNONYMS` (les libellés produits doivent rester canoniques) + `pytest` des deux paquets                                     |
| Modèle exporté (`ai-models/exported/*.joblib`)               | Régénérable (gitignoré) ; `ai-service` charge le bundle au boot — `metadata.json` (suivi) fait foi des métriques/versions ; cf. ADR-030                                                                 |
| Table de routage gateway (`api-gateway/.../proxy.routes.ts`) | Ajouter aussi le service critique au readiness `health.controller.ts` + `*_SERVICE_URL` dans `env.schema.ts` + `README.md` du service (cf. ADR-029)                                                     |
