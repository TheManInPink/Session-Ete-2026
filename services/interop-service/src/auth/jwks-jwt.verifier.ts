 ...
 *              `node:crypto` → cache en Map<kid, KeyObject>. À chaque
 *              vérification : décode header→kid, vérifie RS256, exp/nbf/sub,
 *              projette en `AuthSubject`. Aucune clé privée ici (ADR-027 :
 *              verifier local au service, pattern identique à audit-service).
 *
 *              ⚠️ Ce JWT interne ne protège QUE les routes d'administration /
 *              sortantes (opérateur Mali). Le protocole BCID-AES lui-même
 *              n'utilise PAS ce JWT (mTLS + JWS Ed25519 du pair).
 *
 * @author      Étudiant UQAR
 * @date        2026
 * @module      interop-service/auth
 */
