 ...
 *              auto) pour publier les événements d'audit vers audit-service. Une
 *              seule connexion TCP, fermée proprement au SIGTERM. Désactivable
 *              (test/CI) via `GOVERNANCE_AUDIT_ENABLED=false`.
 *
...
export class RabbitConnection implements OnApplicationShutdown {
  ...
  constructor(cfg: ConfigService<Env, true>) {
    this.enabled = cfg.get('GOVERNANCE_AUDIT_ENABLED', { infer: true });
    this.url = cfg.get('RABBITMQ_URL', { infer: true });
  }

  /** Active si la publication d'audit RabbitMQ est autorisée (désactivable en test). */
  isEnabled(): boolean {
    return this.enabled;
  }
  ...
  get(): AmqpConnectionManager {
    if (!this.enabled) {
      throw new Error('RabbitMQ désactivé (GOVERNANCE_AUDIT_ENABLED=false)');
    }
    if (!this.conn) {
      this.conn = connect([this.url], { heartbeatIntervalInSeconds: 5, reconnectTimeInSeconds: 5 });
      this.conn.on('connect', () => this.logger.log('RabbitMQ connecté (audit)'));
