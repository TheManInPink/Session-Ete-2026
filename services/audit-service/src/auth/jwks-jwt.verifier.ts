 ...
 *              `JwtVerifier`) :
 *                1. Décode le header → `kid`.
 *                2. Récupère la KeyObject correspondante (sinon 401).
 *                3. Vérifie la signature RS256 (crypto.verify).
 *                4. Vérifie exp / nbf / sub.
 *                5. Projette le payload en `AuthSubject`.
 *
 *              Pattern IDENTIQUE à document-service (cohérence inter-services,
 *              ADR-027 : verifier local au service). Aucune clé privée ici.
 *
 * @author      Étudiant UQAR
 * @date        2026
 * @module      audit-service/auth
 */
