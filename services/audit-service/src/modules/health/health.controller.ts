 ...
 * @description Healthcheck Terminus — Postgres (dépendance critique).
 *
 *                GET /health        — détaillé (exclu du préfixe api/v1)
 *                GET /health/live   — liveness (process up)
 *                GET /health/ready  — readiness (Postgres requis)
 *
 *              Marqué `@Public()` (défensif : aucun guard global ici, mais
 *              cohérent inter-services et à l'épreuve d'un futur APP_GUARD).
 *
 * @author      Étudiant UQAR
 * @date        2026
 * @module      audit-service/modules/health
 */
...
export class HealthController {
  ...
  live(): { status: 'live'; service: 'audit-service'; timestamp: string } {
    return { status: 'live', service: 'audit-service', timestamp: new Date().toISOString() };
  }
