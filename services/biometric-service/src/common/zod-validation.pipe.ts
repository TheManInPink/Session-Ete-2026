 ...
 * @description Pipe de validation Zod réutilisable. Les DTO du biometric-service
 *              sont définis comme schémas Zod (`*.schema.ts` des dossiers `dto/`) ;
 *              ce pipe les applique par paramètre
 *              (`@Body(new ZodValidationPipe(schema))`) et lève une
 *              `BadRequestException` 400 avec le détail des erreurs en cas
 *              d'entrée invalide.
 *
 *              Complémentaire au `ValidationPipe` global (whitelist +
 *              forbidNonWhitelisted) : Zod assure ici un parse strict (rejet des
 *              clés inconnues via `.strict()` côté schéma) et une coercition
 *              typée déterministe.
 *
