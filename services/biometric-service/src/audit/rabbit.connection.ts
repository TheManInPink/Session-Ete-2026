 ...
 *              auto) pour publier les événements d'audit biométrique. Une seule
 *              connexion TCP, fermée proprement au SIGTERM. Désactivable (test/CI)
 *              via `BIOMETRIC_AUDIT_ENABLED=false`.
 *
...
export class RabbitConnection implements OnApplicationShutdown {
  ...
  constructor(cfg: ConfigService<Env, true>) {
    this.enabled = cfg.get('BIOMETRIC_AUDIT_ENABLED', { infer: true });
  ...
  get(): AmqpConnectionManager {
    if (!this.enabled) {
      throw new Error('RabbitMQ désactivé (BIOMETRIC_AUDIT_ENABLED=false)');
    }
    if (!this.conn) {
