...
def test_verify_bearer_accepts_valid_rs256_token() -> None:
    """Un JWT RS256 signé par la bonne clé est accepté et décodé."""
    priv, pub = _keypair()
    token = jwt.encode({"sub": "u1", "realm_access": {"roles": ["admin"]}}, priv, algorithm="RS256")
...
def test_verify_bearer_rejects_wrong_signature() -> None:
    ...
    priv_a, _ = _keypair()
    _, pub_b = _keypair()
    token = jwt.encode({"sub": "u1"}, priv_a, algorithm="RS256")
