 ...
 *
 *              C'est le cœur de la responsabilité n°2 du gateway :
 *              « Vérifie le JWT UNE SEULE FOIS » — la signature RS256 est
 *              validée ici, à la frontière. Les services aval n'ont alors plus
 *              qu'à faire confiance au header `X-User-Context` signé par le
 *              gateway (cf. {@link UserContextSigner}).
 *
 *              Au boot : fetch JWKS → convertit chaque JWK RSA en KeyObject
 *              `node:crypto` → cache en Map<kid, KeyObject>. Aucune clé privée
 *              ici (ADR-027 : verifier local au service, jamais dans un package
 *              partagé pour éviter la duplication de `@nestjs/core`).
 *
...
export class JwksJwtVerifier implements JwtVerifier, OnModuleInit {
    ...
    try {
      ...
      this.log.warn(
        `JWKS indisponible (${this.url}) : ${(err as Error).message} — ` +
          "les endpoints protégés répondront 401 jusqu'à la disponibilité d'auth-service.",
      );
    }
  }

  /** Nombre de clés actuellement en cache (exposé pour /health & introspection). */
  get keyCount(): number {
   ...
   *
   * Stratégie d'erreur UNIFORME (`AUTH_TOKEN_INVALID`) quel que soit le sous-cas
   * (signature, expiration, kid inconnu) : empêche un oracle d'énumération.
   *
